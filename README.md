...
公開URL: https://hare.sirumo.com/ （旧Pagesプロジェクト `hare-dayori` で配信継続）。GitHub自動デプロイ先は https://hare-dayori-git.pages.dev/ （`hare-dayori-git`、本番ブランチ `main`）。独自ドメインの接続先移行はDNS操作待ちです。独自ドメインは接続済みです。運営者情報が未確定のため、noindex付きの先行公開として運用します。表示名は仮称。広告・アクセス解析は追加していません。
...
初回のGit自動デプロイは成功しました。コミット `dfa70a2`、トリガー `github:push`、デプロイID `0dd393ab-7358-4e90-bcdf-970671b81440`。GitHub Actionsも25テスト成功。新しい `pages.dev` URLでHTTPS・記事表示・検索操作・スマホ表示・robots.txt・noindexを確認しました。`hare.sirumo.com` はまだ旧プロジェクトにつながっており、Gitへのpushが独自ドメインへ反映される状態ではありません。DNS変更とPagesのドメイン付け替えをまとめて行ってから移行完了とします。
